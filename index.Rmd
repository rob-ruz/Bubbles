...
global_daily1 %>% 
  filter(conm == "INDUSTRIAS PENOLES SAB DE CV", datadate == "2004-04-16")

global_daily1 %>% 
  filter(conm == "CEMEX SAB DE CV", datadate == "2019-06-13")
...
```{r}
global_daily1 %>%
  filter(conm == "TUBOS DE ACERO DE MEXICO") %>% 
  group_by(curcdd) %>% 
  summarise(min = min(datadate), max = max(datadate))
```
