...
Importamos la clasificación de sectores de la BMV.
...
Enseguida vamos a generar dos data frames. Uno contiene a las observaciones que son fin de mes (DatosMX02), y el otro al resto de observaciones (DFnoFinMes). Del segundo de estos data frames, identificaremos aquellos "gvkey_iid" + "año-mes" que no tienen un identificador igual en el data frame de fin de mes; estas observaciones las utilizaremos para insertarlas en DatosMX02.
...
DatosMX02 %>% group_by(gvkey_iid,month = format(datadate, "%Y%m")) %>% summarise(n=n())%>% filter(n>1) %>% spread(month,n) # NO hay "gvkey_iid" con más de un "fin de mes".
...
Rangofechas <- DatosMX02 %>% group_by(gvkey_iid) %>% 
...
DFnoFinMes <- DFnoFinMes %>% 
  ...
  anti_join((DatosMX02 %>% mutate(tempID=paste(gvkey, iid,format(datadate, "%Y-%m"), sep = "_"))), by="tempID") %>% 
  select(-tempID)
...
DFnoFinMes <- DFnoFinMes %>% inner_join(Rangofechas,by="gvkey_iid") %>% rowwise() %>%
...
DatosMX03 <- Rangofechas %>% gather(Rango,Fechas,2:3) %>% arrange(gvkey_iid)
...
DatosMX03 <- DatosMX03 %>% group_by(gvkey_iid) %>%
  complete(Fechas=seq.Date(ceiling_date(min(Fechas),"month"), ceiling_date(max(Fechas),"month"), by="months")-1)%>%
  select(-Rango)
# Pasamos las observaciones de DatosMX02 a DatosMX03.
DatosMX03 <- DatosMX03 %>% 
  mutate(tempID=paste(gvkey_iid,format(Fechas, "%Y-%m"), sep = "_")) %>% 
  left_join((DatosMX02 %>% mutate(tempID=paste(gvkey_iid,format(datadate, "%Y-%m"), sep = "_")) %>%
  select (-gvkey_iid)), by="tempID") %>%
  select (-tempID)
# Llenamos entonces los "missing values".
DatosMX03<- DatosMX03 %>% group_by(gvkey_iid) %>% fill(3,4,6,8,9,11,12,14,20,22,23)
...
**LAS LÍNEAS QUE SIGUEN NO FUERON MODIFICADAS. Los missing values y otros aspectos que se abordan en las siguientes líneas se encuentran en la propuesta de modificación de las líneas de arriba.** 
