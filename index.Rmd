...
```{r}
# de los gvkey que tienen mas de una emisión... cual es el valor de mercado promedio de cada una por empresa
DatosMX01 %>%
  group_by(gvkey) %>%
  filter(n_distinct(iid) > 1) %>%
  group_split() %>%
  modify(~ group_by(.x, conm, iid) %>% summarise(prom_vm = mean(Size))) 
```
