...
lagged_returns <- function(var) {
  ...
  map(indices, ~ quo(!!var / lag(!!var, !!.x) - 1)) %>%
    set_names(sprintf("mkt_return_%02d", indices))
}
...
datos_mensuales %>%
  ...
  arrange(conm, datadate) %>%
  group_split() %>%
  set_names(., map_chr(., ~ .x$BMV[[1]])) %>%
  map(
    ~ index_by(.) %>% summarise(sector_rend = mean(return, na.rm = T),
                                sector_rend5 = mean(return5, na.rm = T))
  ) %>%
  map(
    ~ left_join(
      .,
      mkt_index %>% mutate(mkt_rend = prccm / lag(prccm, !!lag) - 1) %>% select(-prccm)
    ) %>%
