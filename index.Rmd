...
keep_emisoras <- DatosMX01 %>% 
  group_by(gvkey, iid) %>%
  summarise(prom_mv = mean(Size, na.rm = TRUE)) %>% 
...
DatosMX01 %>%
  mutate(
    year = year(datadate),
    month = paste(month(datadate, label = TRUE, locale = "US"), year(datadate))
  ) %>%
  group_by(year) %>%
  group_split() %>%
  modify(
    ~ group_by(.x, gvkey_iid, conm, month) %>%
...
DatosMX01 %>%
  ...
  fill_gaps() %>% tidyr::fill(everything(), .direction = "down")
```
